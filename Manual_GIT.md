...
* В папке создаётся скрытая папка .git
 ...
 1.5.1  На github находим интересующий нас репозиторий, нажимаем fork (правый верхний угол). Получаем копию репизитория и находимся у себя на странице, о чем гласит сноска в левом верхнем углу _forked from TimurIslamgulov/DZ3_
  
 1.5.2 Нажимаем CODE, копируем ссылку.
  
  Например **https://github.com/KomissarGibert/DZ3.git**

 1.5.3 В VSC делаем **git clone** копию репозитория. Создаем новую папку (не обязательно)
 ...
 1.5.4  Отправляем свою версию в свой GitHub:
    - *__git push__*
    - *__git push --set-upstream origin__*

   Например *__git push --set-upstream origin__* _Push_Request_G_

1.5.5 На сайте GitHub появляется надпись *Push_Request_G had recent pushes less than a minute ago* и новая ветка по мимо master, в нашем случае *Push_Request_G* нажимаем кнопку pull request 
...
11. Файл *__.gitignore__* Добавляем еще файл (+ создать файл) в наименовании указываем имя файл + расширение, например "photo2.jpg". Далее "git add .gitignore" и git commit -m "Добавили gitignore". Созданный файл должен стать серым.
